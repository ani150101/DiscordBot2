    ...
    run: async (client, message, args) => {
        const helpEmbed = (message) => {
            let helpFields = [
                ...
                {name: `user`, value: `Gets a user's information.\n\`\`${process.env.PREFIX}user / ${process.env.PREFIX}user <member/member ID>\`\``, inline: true},
                ...
                {name: `embed, emb, e`, value: `Send an embedded message.\n\`\`${process.env.PREFIX}embed <message>\`\``, inline: true},
                {name: `say, send`, value: `Send a message.\n\`\`${process.env.PREFIX}say <message>\`\``, inline: true},
            ]
            richEmbed(message, false, "List of Bot Commands", false, true, message.guild.iconURL({dynamic: true}), helpFields, 'BLUE');
        }
